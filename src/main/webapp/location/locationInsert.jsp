...
<body>
		...
		<form action="./locationInsertAction.lo" method="post">
			<table>
				<tr>
					...
					<td><input type="text" name="location_name"
						placeholder="수취인 성함" value="${dto.location_name}"
				...
				<tr>
					...
					<td>
						...
						<input id="location_phone0" type="text" name="location_phone[]"
						pattern="\d*" maxlength="3"
						value="${dto.location_phone.substring(0,3)}" required="required">
						<label> - </label> <input id="location_phone1" type="text"
						name="location_phone[]" pattern="\d*" maxlength="4"
						value="${dto.location_phone.substring(3,7)}" required="required">
						<label> - </label> <input id="location_phone2" type="text"
						name="location_phone[]" pattern="\d*" maxlength="4"
						value="${dto.location_phone.substring(7,11)}" required="required">
					</td>
				</tr>
				<tr>
					<td><label>배송지우편번호 : </label></td>
					<td><input type="text" id="sample6_postcode"
						placeholder="우편번호" name="location_postcode"
						value="${dto.location_postcode}" readonly="readonly"> <input
				...
				<tr>
					...
					<td><input type="text" id="sample6_address" placeholder="주소"
						name="location_add" value="${dto.location_add}"
						readonly="readonly"></td>
				</tr>
				<tr>
					<td><label>상세주소 : </label></td>
					<td><input type="text" id="sample6_extraAddress"
						placeholder="상세주소" name="locationD_add"
						value="${dto.locationD_add}" required="required"></td>
				</tr>
				<tr>
					<td><label>배송지이름 : </label></td>
					<td><input type="text" name="location_title"
						placeholder="배송지 이름" value="${dto.location_title}"
						required="required"></td>
				</tr>
				<tr>
					<td><label>배송요청사항 : </label></td>
					<td><select id="sample6_detailAddress"
						name="location_requested">
							...
							<option label="파손의 위험이 있는 상품입니다.배송 시 주의해 주세요"
								value="파손의 위험이 있는 상품입니다.배송 시 주의해 주세요">
							<option label="직접 입력">
					</select> <br> <textarea id="directInput" rows="3" cols="40" hidden=""></textarea>
					</td>
				</tr>
			</table>
			<input type="hidden" name="member_id" value="1"> <input
				type="hidden" name="location_id" value="${dto.location_id}">
	...
	<script type="text/javascript">
		$(function() {
			$("#sample6_detailAddress")
					.change(
							function() {
								var result = $(
										'#sample6_detailAddress option:selected')
										.attr("label") == '직접 입력';
								if (result) {//직접입력 체크
		...
		function validityCheck() {//우편번호,주소,배송요청사항 유효성 체크
			var result = $('#sample6_detailAddress option:selected').attr(
					"label") == '직접 입력';
			...
			if (phone0.length < 3 || phone1.length < 4 || phone2.length < 4) {//전화번호 유효성 체크
				...
				$("input[name='location_phone']").attr("value",
						phoneArr.join(""));
			}

			if (result) {//직접 입력 체크
				$('#sample6_detailAddress option:selected').attr("value",
						$("#directInput").val());
			}

			if ($("#sample6_postcode").val() == ""
					|| $("#sample6_address").val() == "") {//우편번호,주소 체크
				alert("우편번호를 확인해주세요");
				return false;
			}
		}
